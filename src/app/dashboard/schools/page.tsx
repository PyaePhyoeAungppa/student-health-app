        ...
        <div>
            <div className="page-header flex flex-col sm:flex-row gap-4">
                <div className="w-full sm:w-auto">
                    ...
                    <p className="text-muted-foreground text-sm mt-1">{schools.length} total schools registered</p>
                </div>
                {role === "SYSTEM_ADMIN" && (
            ...
            {showAddForm && role === "SYSTEM_ADMIN" && (
                <div className="glass-card mb-6 animate-fade-in p-6">
                    <h2 className="font-semibold text-sm uppercase tracking-wider text-muted-foreground mb-4">Add New School</h2>
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="md:col-span-2 flex justify-end gap-3 mt-2">
                            <button type="button" onClick={() => setShowAddForm(false)} className="px-4 py-2 rounded-lg text-sm font-medium border border-border hover:bg-secondary transition-colors">Cancel</button>
                            <button type="submit" disabled={adding} className="px-4 py-2 rounded-lg text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90 transition-colors flex items-center gap-2">
                                {adding ? <><Loader2 className="w-4 h-4 animate-spin" /> Saving...</> : "Save School"}
                            </button>
                        </div>
                    </form>
                </div>
                ...
                {loading ? (
                    ...
                    <div key={school.id} className="glass-card p-6 flex flex-col justify-between group hover:border-primary/50 transition-all">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground mb-4">{school.province || "Province not specified"}</p>
                            {school.address && (
                        ...
                        <div className="text-xs text-muted-foreground border-t border-border/50 pt-3 mt-2">
                            Registered on {formatDateEn(school.createdAt)}
                        </div>
                    </div>
                ))}
            </div>
